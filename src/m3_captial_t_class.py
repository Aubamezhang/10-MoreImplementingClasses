...
class CapitalT(object):
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        """
        c1h = rg.Point(intersection_center.x - width/2, intersection_center.y - letter_thickness/2)
        c2h = rg.Point(intersection_center.x + width/2, intersection_center.y + letter_thickness/2)
        c1v = rg.Point(intersection_center.x - letter_thickness/2, intersection_center.y - letter_thickness/2)
        c2v = rg.Point(intersection_center.x + letter_thickness/2, intersection_center.y - letter_thickness/2 + height)
        self.h_rect = rg.Rectangle(c1h, c2h)
        self.v_rect = rg.Rectangle(c1v, c2v)
        # --------------------------------------------------------------
        # DONE: 3.
